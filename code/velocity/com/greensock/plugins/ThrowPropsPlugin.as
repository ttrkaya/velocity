                                ...
                                if (p != "resistance" && p != "checkpoint") {
                                        ...
                                        if (typeof(curProp) == "number") {
                                                ...
                                                if ("max" in curProp && end > Number(curProp.max)) {
                                                        //if the velocity is too low, the duration can end up being uncomfortably long but in most situations, users want the snapping to occur relatively quickly (0.75 seconds), so we implement a cap here to make things more intuitive.
                                                        curClippedDuration = (Math.abs(curVelocity) < 10) ? 0.75 : calculateDuration(target[p], curProp.max, curVelocity, ease, checkpoint);
                                                        ...
                                                        //if the velocity is too low, the duration can end up being uncomfortably long but in most situations, users want the snapping to occur relatively quickly (0.75 seconds), so we implement a cap here to make things more intuitive.
                                                        curClippedDuration = (Math.abs(curVelocity) < 10) ? 0.75 : calculateDuration(target[p], curProp.min, curVelocity, ease, checkpoint);
                                                        if (curClippedDuration + overshootTolerance < clippedDuration) {
                                                                clippedDuration = curClippedDuration + overshootTolerance;
                                                        }
                                                }
